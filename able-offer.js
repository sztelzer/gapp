(function() {
	'use strict';
	angular.module('able').directive('offerElement', offerDirective)
	...
	function offerController($rootScope, $localStorage, config, auth, $http) {
		...
		function loadNewOffer() {
			...
			$http.post(config.api + '/users/' + auth.id + '/offers', payload, req_config).then(function successCallback(response) {
		...
		function updateOfferStocks(offerPath) {
			...
			$http.get(config.api + offerPath, req_config).then(function successCallback(response) {
				if (!$rootScope.offer) {
