(function() {
	'use strict';
	angular.module('able').directive('vouchersElement', vouchersDirective)
	...
	function vouchersController($localStorage, $rootScope, $scope, auth, $http, config, $q) {
		...
		function save() {
			...
			$http.post(config.api + '/users/' + auth.id + '/vouchers', payload, req_config).then(function successCallback(response) {
